...
export class OrderResponseDTO {
    ...
    static from(order: OrderEntity, items: OrderItemEntity[], payment? : OrderResponsePaymentInfo): OrderResponseDTO {
        ...
        dto.items = items.map(item => OrderItemResponseDTO.from(item));
...
export class OrderDetailsResponseDTO {
    ...
    static from(order: OrderEntity, items: OrderItemEntity[]): OrderDetailsResponseDTO {
        const dto = new OrderDetailsResponseDTO();
        dto.order = OrderResponseDTO.from(order, items);
        // Object.assign(dto, dto.order );
